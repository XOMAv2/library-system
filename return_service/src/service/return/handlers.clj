(ns service.return.handlers
  (:require [service.return.tables.user-limit :as ulops]
...
(defn get-user-limit
  [{{{:keys [uid]}                  :path}   :parameters
    {{user-limit-table :user-limit} :tables} :db}]
  (if-let [user-limit (ulops/-get user-limit-table uid)]
    {:status 200
     :body user-limit}
    {:status 404
...
(defn get-user-limit-by-user-uid
  [{{{:keys [user-uid]}             :path}   :parameters
    {{user-limit-table :user-limit} :tables} :db}]
  (if-let [user-limit (ulops/-get-by-user-uid user-limit-table user-uid)]
    {:status 200
     :body user-limit}
    {:status 404
...
(defn get-all-user-limits
  [{{{user-limit-table :user-limit} :tables} :db}]
  (let [user-limits (ulops/-get-all user-limit-table)]
    {:status 200
...
(defn update-user-limit
  [{{{:keys [uid]}                  :path
     user-limit                     :body}   :parameters
    {{user-limit-table :user-limit} :tables} :db}]
  (try (if-let [user-limit (ulops/-update user-limit-table uid user-limit)]
         {:status 200
          :body user-limit}
         {:status 404
          :body {:message (str "User limit with uid `" uid "` is not found.")}})
...
(defn update-total-limit
  [{{{:keys [user-uid delta]}       :path}   :parameters
    {{user-limit-table :user-limit} :tables} :db}]
  (try (if-let [user-limit (ulops/-update-total-limit-by-user-uid
...
(defn update-available-limit
  [{{{:keys [user-uid delta]}       :path}   :parameters
    {{user-limit-table :user-limit} :tables} :db}]
  (try (if-let [user-limit (ulops/-update-available-limit-by-user-uid
                            user-limit-table user-uid delta)]
         {:status 200
          :body user-limit}
         {:status 404
...
(defn delete-user-limit
  [{{{:keys [uid]}                  :path}   :parameters
    {{user-limit-table :user-limit} :tables} :db}]
  (if-let [user-limit (ulops/-delete user-limit-table uid)]
    {:status 200
     :body user-limit}
    {:status 404
     :body {:message (str "User limit with uid `" uid "` is not found.")}}))
